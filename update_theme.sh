...
then
	...
	then
		...
		then
			~/.edxthemevenv/bin/ansible-playbook -i ",$4" main.yml -e "mode=$1" -e "theme_branch=$2 theme_repo=$3"
        elif [ "$1" == "$mode_branch_custom_repo_remote_group_edx_host" ];
        then
        	~/.edxthemevenv/bin/ansible-playbook -i $INI_FILE main.yml --limit $4 -e "mode=$1" -e "theme_branch=$2 theme_repo=$3"
		fi
	fi
